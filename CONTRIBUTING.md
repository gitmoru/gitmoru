...
이슈를 열고, 브랜치를 따고, PR 로 머지합니다. 혼자 만드는 저장소에서도 그렇게 해요.

```bash
git switch -c fix/무엇을-고치는지
```

바로 미는 게 빠른 건 맞습니다. 대신 무엇이 왜 바뀌었는지가 커밋 한 줄에만 남아요.
이 도구는 남의 저장소 권한을 쓰고, 되돌리기는 취소가 안 됩니다.
**그런 코드가 아무 기록 없이 들어오는 문을 열어두지 않기로 했습니다.**

릴리스 PR 에는 검사가 안 붙습니다. 로봇이 연 PR 은 다른 워크플로를 못 깨우거든요.
그렇다고 안 본 채로 나가지는 않아요. 배포 작업이 `pnpm check` 를 먼저 통과해야 npm 으로 넘어갑니다.
