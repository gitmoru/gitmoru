...
export const ko = {
  ...
  changeList: {
    empty: '아직 안 팠어요',
    firstRun: [
      ['어디를 파볼지 고르세요', '조직 이름만 넣어도 됩니다. 저장소를 콕 집으면 훨씬 빨라요.'],
      ['시간대를 정하세요', '기본값은 오늘 새벽 0시부터 7시까지예요. 언제인지 알면 그 시간대로 바꿔주세요.'],
      ['파보기를 누르면 여기에 뜹니다', '규칙에 안 걸린 파일도 전부 남아요. 그게 이 도구의 결과물입니다.'],
  ...
  access: {
    ...
    why: {
      ...
      runner: '워크플로가 누군가의 실제 컴퓨터에서 돕니다. 계정을 잠그고 브랜치를 되돌려도 그대로 남아요.',
      secret: '값은 안 가져옵니다. 언제 바뀌었는지만 봐요. 배포 비밀이 갈아끼워지면 정상 코드가 남의 인프라로 나가는데 코드에는 흔적이 안 남습니다.'
    },
    recentTitle: (days: number) => `최근 ${days}일 안에 생긴 것`,
    nothingRecent: (days: number) => `최근 ${days}일 안에 새로 생긴 건 없었어요.`,
    existingTitle: '그 밖에 이미 있던 것',
    undatedTitle: '언제 생겼는지 모르는 것',
    undatedNote: 'GitHub 이 등록 시각을 안 알려줘요. 이번 사고 때 생긴 건지 원래 있던 건지 저희도 모릅니다. 어느 쪽으로도 접지 않고 그대로 둡니다.',
    none: '없음',
