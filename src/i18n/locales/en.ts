...
export const en: Dict = {
  ...
  access: {
    ...
    why: {
      ...
      runner: "Workflow code runs on somebody's actual machine. Locking the account and restoring branches leaves it exactly where it is.",
      secret: 'Values are never fetched, only when they last changed. Swap a deploy secret and every build after it sends correct code to the wrong place, with nothing in the diff.',
    },
    recentTitle: (days) => `Created in the last ${days} days`,
    nothingRecent: (days) => `Nothing new in the last ${days} days.`,
    undatedTitle: 'No registration time available',
    undatedNote:
      'GitHub does not report when these were registered. Whether they appeared during the incident or were always there is not something we can tell, so they are left in neither pile.',
