...
export const ja: Dict = {
  ...
  scanPanel: {
    ...
    rulesNoteStrong: '何を検査するかは、上で選んだリポジトリと期間で決まります。',
    rulesNote: 'この検出は結果に印をつけるだけなので、オフにしても検査する範囲は変わりません。',
    scan: '掘る',
    scanning: '掘っています...',
  },
  ...
  changeList: {
    empty: 'まだ調べていません',
    firstRun: [
      ['どこを掘るか選びます', '組織名だけでも構いません。リポジトリを指定するとずっと速くなります。'],
      ['期間を決めます', '既定は今日の 0 時から 7 時までです。時刻が分かっているなら、その期間に変えてください。'],
      ['掘るを押すとここに出ます', 'ルールにかからなかったファイルもすべて残ります。それがこのツールの成果物です。'],
  ...
  verdict: {
    ...
    exposedTitle: (repos, forked) =>
      forked === 0
        ? `リポジトリ ${repos} 件が公開に変わりました`
        : forked === repos
          ? `リポジトリ ${repos} 件がフォークで持ち出されました`
          : `リポジトリ ${repos} 件が外に出ました（うち ${forked} 件はフォーク）`,
    exposedDetail: (files) =>
  ...
  usage: {
    calls: (n) => `GitHub へのリクエスト ${n} 回`,
    saved: (n) => `キャッシュで ${n} 回節約`,
    remaining: (n) => `この 1 時間の残り ${n} 回`,
    low: '1 時間あたりの残り回数がわずかです。今もう一度スキャンすると途中で止まり、「確認できません」ばかりになる可能性があります。1 時間ごとに回復します。',
  },
  ...
  share: {
    ...
    exposed: (n) =>
      `非公開だったリポジトリ ${n} 件が、この期間に外へ出ました（公開への変更、またはフォーク）。誰が持ち出したかを確認する手段はないため、中にあった鍵やトークンは発行し直してください。`,
    added: (n) =>
      `この期間にリポジトリへ追加された人が ${n} 名います。元のアカウントを止めてもこれは閉じず、ブランチを戻しても残ります。`,
    workflow: (n) =>
  ...
  access: {
    ...
    kinds: {
      ...
      invitation: '保留中の招待',
      runner: 'セルフホストランナー',
      secret: 'Actions シークレット',
    },
    why: {
      deployKey: 'アカウントを止めても、この鍵が生きていれば入れます。',
      webhook: 'コミットのたびにリポジトリの内容がこの宛先へ送られます。',
      invitation: 'まだ承諾前なので、今なら取り消せます。',
      runner: 'ワークフローのコードが誰かの実機で動きます。アカウントを止めてブランチを戻しても、そのまま残ります。',
      secret: '値は取得しません。最後にいつ変わったかだけを見ます。デプロイ用の秘密が差し替えられると、正しいコードが別の場所へ送られ、diff には何も残りません。',
    },
    recentTitle: (days) => `直近 ${days} 日以内にできたもの`,
    nothingRecent: (days) => `直近 ${days} 日以内に新しくできたものはありません。`,
    undatedTitle: '登録された時刻が分からないもの',
    undatedNote:
      'GitHub が登録時刻を返しません。今回の事故で増えたのか元からあったのか、こちらでも分かりません。どちらにも寄せずそのまま置きます。',
    ...
    orgHookWhyNotAsk:
      'GitHub にはこの権限の読み取り専用版がありません。付与すると組織 Webhook の作成と削除までできるようになり、読み取りだけのツールが求める権限ではないと判断しました。必要なら手動で付与し、確認後に戻していただけます。',
  ...
  detectors: {
    ...
    pinLoosened: {
      name: 'アクションの固定が外れた箇所',
      rationale: 'ワークフローが使うアクションがコミットハッシュからタグに変わっていないかを見ます。タグは持ち主がいつでも動かせます。',
      title: 'アクションの参照先が固定からタグに変わりました',
      summary: (count, path) =>
        `${path} でアクション ${count} 件が、コミットハッシュではなくタグを指すようになりました。タグはそのアクションの持ち主が別のコミットへ動かせます。`,
      change: (before, after) => `${before}... から ${after} へ`,
      whyLabel: 'なぜ重要か',
      why: '2025 年 3 月、広く使われていたアクションがこの経路で乗っ取られました。攻撃者は既定ブランチ以外にコミットを作り、既存のタグをそこへ付け替えました。タグを使っていたリポジトリは何も変更していないのに、次の実行でそのコードを取り込みました。',
      openFile: 'ワークフローを開く',
      readFailed: (reason) => `ワークフローファイルを読めませんでした: ${reason}`,
    },
    workflowRisk: {
      name: 'ワークフローが開けた口',
      rationale: '変更されたワークフローが、他人のコードや秘密への口を開けていないかを見ます。もともと開いていたものは出ません。',
      title: 'ワークフローがこの期間に口をひとつ開けました',
      summary: (path, count) =>
        `${path} に、以前はなかった箇所が ${count} 件できました。次の実行からはここに書かれたとおりに動きます。`,
      risk: {
        'pr-target-checkout': {
          label: '他人の PR のコードが秘密を持って動きます',
          detail: 'pull_request_target はこのリポジトリの権限と秘密を持って動きます。そこで PR 側のコードをチェックアウトすると、他人が書いたコードがその権限で実行されます。',
        },
        'event-in-run': {
          label: 'PR のタイトルが命令になり得ます',
          detail: 'github.event の値は、シェルが動く前にその場に文字として埋め込まれます。タイトルやブランチ名に命令を書いて送れば、そのまま実行されます。',
        },
        'write-all': {
          label: 'トークンの権限が全開です',
          detail: 'permissions: write-all だと、そのジョブはリポジトリのほとんどを変更できます。',
        },
      },
      limitLabel: 'この検査の限界',
      limit: 'YAML として読まず、行単位で見ています。複数行に散らしたり、アンカーを使われると見えません。出なかったことは、ないことではありません。',
      openFile: 'ワークフローを開く',
      readFailed: (reason) => `ワークフローファイルを読めませんでした: ${reason}`,
    },
    sharedBlob: {
